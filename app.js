...
S.temp = (typeof S.temp === "number") ? S.temp : 0.3;   // AI 온도(창의성) — Gemini/DeepSeek 적용
...
async function aiChat(messages, extra) {  // 프록시로 messages 전송 → 응답 텍스트. extra: {model,image,think}
  ...
  const body = Object.assign({ messages, model: currentModel() }, extra || {});
  const r = await fetch(AI_PROXY, { method: "POST", headers, body: JSON.stringify(body) });
...
function downscaleImage(file, cb) {   // 업로드 전 ~1024px JPEG로 축소(비용·용량 절감)
...
function setPendingImage(d) {
  pendingImage = d || null;
  const box = $("#aiAttach"); if (!box) return;
  if (pendingImage) { box.innerHTML = `<div class="att-thumb"><img src="${pendingImage}" alt="첨부 이미지"><button type="button" class="att-x" id="attX" aria-label="이미지 제거">✕</button></div>`; box.hidden = false; const x = $("#attX"); if (x) x.onclick = () => setPendingImage(null); }
  else { box.innerHTML = ""; box.hidden = true; }
}
function updateThinkBtn() {
  const b = $("#aiThinkBtn"); if (!b) return;
  b.hidden = !/^deepseek/i.test(currentModel());     // thinking 토글은 DeepSeek일 때만
  b.classList.toggle("on", !!S.think);
}
function supportsTemp() { return /^(gemini|deepseek)/i.test(currentModel()); }   // GPT-5 계열은 커스텀 temperature 미지원
function renderAiSettings() {
  const s = $("#aiSettings"); if (!s) return;
  if (supportsTemp()) {
    s.innerHTML = `<div class="ai-set-row"><label for="aiTempRange">🌡️ 온도(창의성)</label>
      <input type="range" id="aiTempRange" min="0" max="2" step="0.1" value="${S.temp}">
      <span id="aiTempVal">${(+S.temp).toFixed(1)}</span></div>
      <div class="ai-set-hint">낮을수록 정확·일관적, 높을수록 창의적. (Gemini·DeepSeek에 적용)</div>`;
    const r = $("#aiTempRange"); if (r) r.oninput = () => { S.temp = parseFloat(r.value); save(); const v = $("#aiTempVal"); if (v) v.textContent = S.temp.toFixed(1); };
  } else {
    s.innerHTML = `<div class="ai-set-hint">현재 모델(${esc(currentModel())})은 온도 조절을 지원하지 않습니다. Gemini나 DeepSeek을 선택하세요.</div>`;
  }
}
...
function chatRender() {
  ...
  else th.innerHTML = CHAT.map(m => `<div class="ai-msg ${m.role}${m.role === "assistant" ? " ai-rich" : ""}">${m.role === "assistant" ? mdRender(m.content) : (m.image ? `<img class="msg-img" src="${m.image}" alt="첨부 이미지">` : "") + esc(m.content).replace(/\n/g, "<br>")}</div>`).join("");
...
async function sendChat() {
  ...
  const text = inp.value.trim(); const img = pendingImage;
  if (!text && !img) return;
  if (!aiConfigured()) { CHAT.push({ role: "assistant", content: "AI 기능이 아직 설정되지 않았습니다(관리자 설정 필요)." }); chatRender(); return; }
  stopType();
  CHAT.push({ role: "user", content: text || "(이미지)", image: img || null }); inp.value = ""; setPendingImage(null); chatRender();
  ...
  if (img) { extra.image = img; extra.model = visionModel(); }    // 이미지는 비전 모델(Gemini)로 라우팅
  else if (/^deepseek/i.test(currentModel()) && S.think) extra.think = true;
  let animate = false;
  try {
    const r = await aiChat(sys.concat(hist), extra);
...
function initChatPanel() {
  ...
  if (pick) {
    if (AI_MODELS.length > 1 && mbtn && menu && curEl) {
      ...
      menu.querySelectorAll(".ai-model-item").forEach(it => it.onclick = () => {
        S.model = it.dataset.mid; save(); setLabel(); updateThinkBtn();
        const sp = $("#aiSettings"); if (sp && !sp.hidden) renderAiSettings();
  ...
  const inp = $("#aiInput"); if (inp) {
    ...
    inp.addEventListener("paste", e => {     // Ctrl+V 이미지 붙여넣기
      const items = e.clipboardData && e.clipboardData.items; if (!items) return;
      for (const it of items) { if (it.type && it.type.indexOf("image") === 0) { const f = it.getAsFile(); if (f) { e.preventDefault(); downscaleImage(f, d => { if (d) setPendingImage(d); else toast("이미지를 읽지 못했어요"); }); return; } } }
    });
  }
  const ab = $("#aiAttachBtn"), file = $("#aiFile");
  if (ab && file) { ab.onclick = () => file.click(); file.onchange = () => { const f = file.files && file.files[0]; if (f) downscaleImage(f, d => { if (d) setPendingImage(d); else toast("이미지를 읽지 못했어요"); }); file.value = ""; }; }
  const tb = $("#aiThinkBtn"); if (tb) tb.onclick = () => { S.think = !S.think; save(); updateThinkBtn(); };
  updateThinkBtn();
  const setBtn = $("#aiSettingsBtn"), setPanel = $("#aiSettings");
  if (setBtn && setPanel) setBtn.onclick = () => { setPanel.hidden = !setPanel.hidden; if (!setPanel.hidden) renderAiSettings(); };
  // 이미지 드래그앤드롭
  if (panel) {
    panel.addEventListener("dragover", e => { e.preventDefault(); panel.classList.add("dragging"); });
    panel.addEventListener("dragleave", e => { if (e.target === panel) panel.classList.remove("dragging"); });
